        ...
        if command == 'host':
            ...
            if algorithm not in cc_algorithms:
                if algorithm not in unknown_algorithms:
                    unknown_algorithms.append(algorithm)
                continue
...
def run_test(commands, directory, name, bandwidth, initial_rtt, buffer_size, buffer_latency, poll_interval):
    ...
    s1.cmd('tc qdisc add dev s1-eth1 root handle 1: tbf rate 1000Mbit buffer 1600 latency 1000ms')
    s1.cmd('tc qdisc add dev s1-eth1 parent 1:1 handle 10: netem delay 1ms limit 1000000')
    s2.cmd('tc qdisc add dev s2-eth2 root handle 1: tbf rate {} buffer 15000 latency {}'.format(
        bandwidth, buffer_latency))

    netem_running = False
    if initial_rtt != '0ms':
        netem_running = True
        s2.cmd('tc qdisc add dev s2-eth2 root handle 1:0 netem delay {}ms'.format(initial_rtt))
    ...
    try:
        for cmd in commands:
            ...
            if cmd['command'] == 'link':
                s1, s2 = net.get('s1', 's2')
                if cmd['change'] == 'bw':
                    s2.cmd('tc qdisc change dev s2-eth2 root tbf rate {} buffer 16000 latency {}'.format(
                        cmd['value'], buffer_latency))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', dest='rtt',
                        default='10ms', help='Initial rtt for all flows. (default 10ms)')
    ...
    parser.add_argument('--poll-interval', dest='poll_interval', type=float,
                        default=0.001, help='Interval to poll TCP values and buffer backlog in seconds. (default: 0.001)')
